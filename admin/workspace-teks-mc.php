...
if (!$kegiatan_id) redirect('admin/dashboard.php', 'ID Kegiatan tidak valid.', 'error');

requireEventAccess($kegiatan_id, ['ketuplat', 'sie_acara']);
$kegiatan = dbFetchOne("SELECT * FROM kegiatan WHERE id = ?", [$kegiatan_id], "i");
if (!$kegiatan) redirect('admin/dashboard.php', 'Kegiatan tidak ditemukan.', 'error');
...
<div class="mc-workspace-container">
    ...
    <?php if ($success_msg): ?>
        <div style="background: rgba(46, 204, 113, 0.1); color: #2ecc71; padding: 15px; border-radius: 12px; border: 1px solid rgba(46, 204, 113, 0.2); margin-bottom: 20px;">
            ...
            <a href="arsip-teks-mc.php" style="color: #4facfe; margin-left: 10px; text-decoration: underline;">Lihat Arsip Teks MC →</a>
        </div>
    <?php endif; ?>
    <?php if ($error_msg): ?>
    ...
    <form method="POST" id="mcForm">
        ...
        <div class="actions-bar">
            <div>
                <a href="arsip-teks-mc.php" style="color: #888; text-decoration: none; font-size: 0.9rem; margin-right: 15px;">
            ...
            <div style="display: flex; gap: 12px; align-items: center;">
                <?php if ($edit_id > 0): ?>
                    <a href="reader-teks-mc.php?id=<?php echo $edit_id; ?>" target="_blank" class="btn-reader" title="Buka Tampilan Teleprompter Layar Penuh">
                        <i class="fas fa-play-circle"></i> Mode Live Reader
                    </a>
                    <a href="cetak-teks-mc-pdf.php?id=<?php echo $edit_id; ?>" target="_blank" class="btn-pdf" title="Cetak Format Cue Card PDF">
